...
@router.callback_query(F.data == "change_default_mute_time", filters.MemberIsAdmin())
async def entry(query: types.CallbackQuery, state: FSMContext):
    ...
    sent = await query.message.edit_text(
        text=translations.get_string(
            "settings.default_mute_time.text", chat.language
        ).format(
            time=(
...
@router.message(DefaultMuteDuration.input)
async def input_(message: types.Message, state: FSMContext):
    ...
    async with database.session() as session:
        await session.execute(
            update(models.Chat)
            .values(default_mute_time=parsed.seconds)
