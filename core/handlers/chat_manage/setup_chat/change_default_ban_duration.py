...
def register_handlers(dp: Dispatcher):
    dp.register_callback_query_handler(
        ...
        lambda q: q.data == "change_default_ban_time",
        filters.MemberIsAdmin()
    )

    dp.register_message_handler(
        set_default_ban_duration,
        state=DefaultBanDuration.input
    )
...
async def change_default_ban_duration(query: types.CallbackQuery, state: FSMContext):
    ...
    sent = await query.message.edit_text(
        text=translations.get_string(
            "settings.default_ban_time.text",
            chat.language
        ).format(
            time=(
...
async def set_default_ban_duration(message: types.Message, state: FSMContext):
    ...
    async with message.bot.db() as session:
        await session.execute(
            update(
                ...
                default_ban_time=parsed.seconds
            ).where(
